...
class CartLogic extends Model
{
    ...
    public function getCartList($selected = 0){
        ...
        $cartList = $cart->field('id, goods_id, goods_num, goods_name, goods_sn,sku, goods_price, member_goods_price, selected,prom_type,prom_id')
                        ->with(['promGoods', 'goods'=>function($query){
                            $query->field('goods_id, goods_name, subtitle, tag, shop_price, original_img, is_on_sale, cat_id, city_code,prom_type,prom_id, store_count');
                        }])
                        ->where($cartWhere)
                        ->order('id desc')
                        ->select();  // 获取购物车商品
...
//        $cartCheckAfterList = $cartList;
    ...
    public function checkCartList($cartList){
        ...
        foreach($cartList as $cartKey=>$cart){
            ...
            // 过滤非用户当前城市的商品
            if($this->city_code && $cart['goods']['city_code'] != $this->city_code){
                $cart->delete();
                unset($cartList[$cartKey]);
                continue;
            }
    ...
    public function changeNum($cart_id, $goods_num){
        ...
        if($this->user_id != $cart->user_id) {
            return ['status' => 0, 'msg' => '非法操作', 'result' => ''];
        }
